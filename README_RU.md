...
- **Окно браузера видно намеренно.** Amazon в любой момент может попросить войти заново или показать проверку. После каждой загрузки утилита ищет известные признаки проверки; если нашла — печатает сообщение и до пяти минут ждёт, пока *ты* пройдёшь её в окне, потом продолжает. Сама она ничего не решает, не проксирует и не подделывает. `--headless` — для сессии, которой уже доверяешь.
