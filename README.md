...
- **Headed on purpose — unlike the Glovo export.** Glovo talks to a JSON API and its bot check only ever appears at sign-in, so `npm run export` runs with no window. Amazon renders pages, and a captcha, a WAF check or a fresh sign-in can appear on any of them, in the middle of a run. After every page load the tool looks for the known challenge markers; on a hit it prints a message and waits up to five minutes for *you* to solve it in the window, then carries on. Nothing is ever solved, proxied or spoofed by the tool.
- **`--headless` once you trust the session.** Same walk, no window. A challenge is still detected, but there is nowhere to solve it: the tool waits five minutes, then fails with an error, and you rerun headed. Do the probe and the first full export headed; switch to `--headless` for later runs if none of them tripped a check. A hidden Chromium is also somewhat more likely to trip one than a visible one.
